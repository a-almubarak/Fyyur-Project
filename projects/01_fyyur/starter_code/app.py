...
def show_venue(venue_id):
    ...
    query1= db.session.query(Artist.id,Artist.name,Artist.image_link,Show.start_time).join(Show).filter(Artist.id==Show.artist_id and today > Show.start_time).all()
    query2= db.session.query(Artist.id,Artist.name,Artist.image_link,Show.start_time).join(Show).filter(Artist.id==Show.artist_id and today < Show.start_time).all()
